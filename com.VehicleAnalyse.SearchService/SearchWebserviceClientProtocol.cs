...
namespace com.VehicleAnalyse.SearchService
{
    ...
    public class SearchWebserviceClientProtocol
    {
        ...
        private WebReferenceSearch.IraWebservice m_server = null;

        public WebReferenceSearch.IraWebservice Server
        {
            get
            {
                if (m_server == null)
        ...
        public void Init(string url)
        {
            try
            {
                ...
                m_server = new WebReferenceSearch.IraWebservice();//webservice设置回档，使用配置文件方式，代码配置方式存在问题，在有的机器上接收8k以上数据
                m_server.Url = url;
        ...
        public string BuildProtocolBody<T>(T args)
        {
            ...
            string body = AppUtil.Common.XMLSerilize.SerilizeObject<T>(args);
